...
(function () {
  ...
  function attachToggle(btn) {
    btn.addEventListener("click", function toggleHandler() {
      var input = document.getElementById(btn.getAttribute("data-target"));
      if (!input) return;
      var isPassword = input.type === "password";
      input.type = isPassword ? "text" : "password";
      /* Replace entire button node to force Edge to drop cached ::before */
      var fresh = btn.cloneNode(true);
      fresh.innerHTML = "";
      var icon = document.createElement("i");
      icon.className = isPassword ? "fa-solid fa-eye-slash" : "fa-regular fa-eye";
      fresh.appendChild(icon);
      btn.parentNode.replaceChild(fresh, btn);
      attachToggle(fresh);
    });
  }
  document.querySelectorAll(".pass-toggle").forEach(attachToggle);
